...
with open(file_path0, "rb") as file:
    btn = st.download_button(
            label="サンプル CSV ファイル 0 ダウンロード(韓国語訳ルビリスト)",
            data=file,
            file_name="韓国語訳ルビリスト_202501.csv",
            mime="text/csv"
        )
...
# 韓国語表示と内部値の対応を辞書で管理
options = {
    'HTML 形式＿ルビサイズ調整': 'HTML格式_Ruby文字_大小调整',
    'HTML 形式＿ルビサイズ調整(漢字置換)': 'HTML格式_Ruby文字_大小调整_汉字替换',
    'HTML 形式': 'HTML格式',
    'HTML 形式(漢字置換)': 'HTML格式_汉字替换',
    '括弧形式': '括弧(号)格式',
    '括弧形式(漢字置換)': '括弧(号)格式_汉字替换',
    '単純置換形式': '替换后文字列のみ(仅)保留(简单替换)'
}

# ユーザーに見せる選択肢（韓国語）
display_options = list(options.keys())

# Streamlit の selectbox を表示
...
#     st.success("CSV 파일이 업로드 및 저장되었습니다.")

#     # ここで JSON アップロードを追加
#     st.write("---")
#     st.write("### JSON 파일 업로드 (선택 사항)")

#     # デフォルトで使用する JSON のパス
#     default_json_path = "./files_needed_to_get_replacements_list_json_format/世界语单词词根分解法user设置.json"
    
#     uploaded_json = st.file_uploader("JSON 파일을 업로드하세요", type=['json'])
    
#     if uploaded_json is not None:
#         # アップロードされた JSON を読み込む
#         change_dec = json.load(uploaded_json)

#         st.success("JSON 파일이 업로드 및 저장되었습니다.")

#     else:
#         st.info("JSON 파일이 업로드되지 않았습니다。デフォルト JSON 을 사용합니다。")
...
if json_choice == "アップロードする":
    ...
    if uploaded_json is not None:
        change_dec = json.load(uploaded_json)
        st.success("JSON 파일을 업로드しました。")
    else:
        st.warning("JSONファイルがアップロードされていません。")
        st.stop()  
        # 同様に、アップロードを期待しているのに無ければ処理ストップか、
        # あるいはデフォルトを使うようにする選択肢もあり。
...
imported_placeholders = import_placeholders('./files_needed_to_get_replacements_list_json_format/占位符(placeholders)_@20374@-@97648@_局部文字列替换用.txt')# placeholderに'@'が含まれる必要は全く無いが、雰囲気を揃えるために敢えて入れた。
...
# JSON文字列に変換（画面上で改行・インデントを見やすくしたいなら indent=2 が便利）
download_data = json.dumps(combined_data, ensure_ascii=False, indent=2)

# --- ダウンロードボタン ---
st.download_button(
    label="Download 最终的な替换用リスト(列表)(合并3个JSON文件)",
    data=download_data,          # ダウンロードするデータ（JSON文字列）
    file_name="最终的な替换用リスト(列表)(合并3个JSON文件).json",  # ユーザーが保存するときのファイル名
    mime='application/json'
)
